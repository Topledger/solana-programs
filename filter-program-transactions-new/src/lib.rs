...
use program_config::create_programs_map;
...
fn populate_transaction_stats(
    ...
    update_transaction_stats_compute_units(transaction_stats, meta);
}
...
fn update_transaction_stats_return_data(
    transaction_stats: &mut TransactionStats,
    meta: &TransactionStatusMeta,
) {
    if let Some(return_data) = &meta.return_data {
        transaction_stats.return_data = Some(transactions::v1::ReturnData {
            program_id: bs58::encode(&return_data.program_id).into_string(),
            data: bs58::encode(&return_data.data).into_string(),
        });
    }
}
...
fn update_transaction_stats_compute_units(
    ...
    if let Some(compute_units) = meta.compute_units_consumed {
        transaction_stats.compute_units_consumed = Some(compute_units as i64);
    }
}
