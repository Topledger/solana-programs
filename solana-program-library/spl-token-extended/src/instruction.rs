...
pub struct InitializeMintLayout {
    ...
    pub freeze_authority: PubkeyLayout, // We'll populate this manually based on the option
}
...
pub fn parse_instruction(bytes_stream: Vec<u8>, accounts: Vec<String>) -> Instruction {
    ...
    match discriminator {
        0 => {
            ...
            // Parse only the fixed part: decimals(1) + mint_authority(32) + freeze_authority_option(1) = 34 bytes
            if rest.len() >= 34 {
                let decimals = rest[0];
                let mut mint_authority = [0u8; 32];
                mint_authority.copy_from_slice(&rest[1..33]);
                let freeze_authority_option = rest[33];
                
                // Handle freeze_authority based on option
                let freeze_authority = if freeze_authority_option == 1 {
                    // Read the next 32 bytes for freeze_authority
                    if rest.len() >= 66 { // 34 + 32
                        let mut authority = [0u8; 32];
                        authority.copy_from_slice(&rest[34..66]);
                        PubkeyLayout { value: authority }
                    } else {
                        PubkeyLayout::default()
                    }
                } else {
                    PubkeyLayout::default() // All zeros when no freeze authority
                };
                
                initializeMintArgs = InitializeMintLayout {
                    decimals,
                    mint_authority: PubkeyLayout { value: mint_authority },
                    freeze_authority_option,
                    freeze_authority,
                };
            } else {
                initializeMintArgs = InitializeMintLayout::default();
            }
        }
        1 => {
