...
use constants::TARGET_MINT_ADDRESS;
        ...
        if let Some(transaction) = trx.transaction {
            ...
            for (idx, inst) in msg.instructions.into_iter().enumerate() {
                ...
                if program == constants::PROGRAM_ADDRESS {
                    ...
                    if let Some(processed_obj) = handle_mints(obj, &pre_token_balances, &accounts, &post_token_balances) {
                        data.push(processed_obj);
                    }
                }
...
fn handle_mints(
    ...
    if obj.instruction_type == "Transfer" || obj.instruction_type == "TransferChecked" {
        if let Some(source) = &obj.input_accounts.source {
            if let Some(index) = accounts.iter().position(|r| r == source) {
        ...
        if let Some(destination) = &obj.input_accounts.destination {
            if let Some(dest_index) = accounts.iter().position(|r| r == destination) {
                for token_balance in post_token_balances.iter() {
                    if token_balance.account_index == dest_index as u32 {
                        obj.input_accounts.destination_owner = Some(token_balance.owner.clone());
                        break;
                    }
                }
            }
        ...
        if obj.input_accounts.mint.as_deref() == Some(TARGET_MINT_ADDRESS) {
            return Some(obj);
        }
