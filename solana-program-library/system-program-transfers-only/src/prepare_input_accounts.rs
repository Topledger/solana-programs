...
pub fn prepare_input_accounts(
    ...
    match instruction_type.as_str() {
        "Transfer" => {
            ...
            result.recipient_account = get_account_with(&input_accounts, 1);
        }
        "TransferWithSeed" => {
            result.funding_account = get_account_with(&input_accounts, 0);
            result.base_account = get_account_with(&input_accounts, 1);
            result.recipient_account = get_account_with(&input_accounts, 2);
        }
        _ => {}
    }
