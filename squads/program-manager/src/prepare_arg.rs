...
pub fn prepare_arg(instruction_data: Vec<u8>, tx_id: String) -> Arg {
    ...
    match arg.instruction_type.as_str() {
        "CreateManagedProgram" => {
            arg.create_managed_program = Some(instruction.createManagedProgram.to_proto_struct());
        }
        "CreateProgramUpgrade" => {
            arg.create_program_upgrade = Some(instruction.createProgramUpgrade.to_proto_struct());
        }
        _ => {}
    }
