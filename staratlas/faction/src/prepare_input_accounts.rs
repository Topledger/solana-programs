...
pub fn prepare_input_accounts(
    ...
    match instruction_type.as_str() {
        "ProcessEnlistPlayer" => {
            result.player_faction_account = get_account_with(&input_accounts, 0);
            result.player_account = get_account_with(&input_accounts, 1);
            result.system_program = get_account_with(&input_accounts, 2);
            result.clock = get_account_with(&input_accounts, 3);
        }
        _ => {}
    }
